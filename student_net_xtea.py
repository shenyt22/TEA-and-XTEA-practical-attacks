...
def train_xtea_distinguisher(num_epochs, nr_range, depth=1, diff=(0x80402010, 0), folder='./', selected_bits=[38, 37, 33, 32, 1, 0]):
    #create the network
    ...
    for _ in nr_range:
      num_rounds += 1
    X, Y = xtea.make_train_data(10**7, nr_range, diff=diff)
    extracted_X = select_tar_bits(X, selected_bits)
    X_eval, Y_eval = xtea.make_train_data(10**6, nr_range, diff=diff)
    extracted_X_eval = select_tar_bits(X_eval, selected_bits)
    #set up model checkpoint
    check = make_checkpoint(wdir+'best'+str(num_rounds)+'depth'+str(depth)+'.h5')
    #create learnrate schedule
    lr = LearningRateScheduler(cyclic_lr(10, 0.002, 0.0001))
    #train and evaluate
    h = net.fit(extracted_X, Y, epochs=num_epochs, batch_size=bs, validation_data=(extracted_X_eval, Y_eval), shuffle=True, callbacks=[lr, check])
